...
def main():
    ...
    stable_label = ""
    stable_count = 0
    stabilization_period =.5  # 2 seconds stabilization period
    min_stable_frames = 2     # Minimum consecutive frames
        ...
        if mode == 2:
                        ...
                        for _ in [1, 2]:
                            ...
                            if results.multi_hand_landmarks is not None:
                                for hand_landmarks, handedness in zip(
                                    ...
                                    logging_csv(imglabel, mode, pre_processed_landmark_list)
                            img = cv.flip(img, 0)
                    except Exception as e:
            ...
            if results.multi_hand_landmarks is not None:
                for hand_landmarks, handedness in zip(
                    ...
                    current_label = keypoint_classifier_labels[hand_sign_id]
                    ...
                    # Only speak if the same gesture is detected for the stabilization period
                    if (stable_count >= min_stable_frames and 
                        current_time - last_detection_time > stabilization_period and
                        current_label != last_spoken_text and 
                        current_label != ""):
                        
                        speak_text(current_label)
                        last_spoken_text = current_label
